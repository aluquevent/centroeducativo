...
<?php
    if(!isset($_GET['id'])){
            ...
            $consulta = $conexion -> prepare("SELECT * FROM alumno WHERE id_alumno=?");
...
?>
    <div class="container">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="nif_alumno" name="nif_alumno" value="<?php echo $datos['nif'];?>">
        </div>
        <div class="form-group">
...
<?php
    ...
    if(isset($_POST['actualizar'])){   
        ...
        $consulta_update = $conexion -> prepare("UPDATE alumno SET nif=?, nombre=?, apellido1=?, apellido2=?, email=?, f_nac=?, imagen=? WHERE id_alumno=?");
        ...
        if($_FILES['imagen']['tmp_name'] != ""){            
            ...
            if(!file_exists("../../assets/img/alumnos")){
                mkdir("../../assets/img/alumnos");
            }
            ...
            $nombre_imagen="foto_alumno_$id".$extension_imagen;
            move_uploaded_file($nombre_tmp_imagen,"../../assets/img/alumnos/$nombre_imagen");

            $imagen=$nombre_imagen;
        }

        $consulta_update -> bindParam(1, $nif);
