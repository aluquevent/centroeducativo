...
<!-- Button trigger modal -->
  ...
  <main>
    <?php
    if(isset($_GET['id'])){
        $consulta_noticia = $conexion -> prepare("SELECT * FROM noticia WHERE id=?");
        $id = $_GET['id'];
        $consulta_noticia -> bindParam(1, $id);
        $consulta_noticia -> setFetchMode(PDO::FETCH_ASSOC);
        $consulta_noticia -> execute();
        $datos_noticia = $consulta_noticia -> fetch();

        ?>
        <div class="container">
            <div class="row text-center">
                <div class="col-12 text-center">
                    <img style="border-radius:50px;"src="assets/img/noticias/thumbnail/<?php echo "$datos_noticia[thumbnail]"?>" alt="">
                    <h1><?php echo "$datos_noticia[titulo]"?></h1>

                </div>
                <div class="col-6">
                  <h3>Día</h3>
                <p><?php echo devuelveDia($datos_noticia['fecha']); echo" "; echo devuelveMes($datos_noticia['fecha'])?></p>
                </div>
                <div class="col-6">
                  <h3>Hora</h3>
                  <p><?php echo devuelveHoraMinutos($datos_noticia['hora_inicio']); echo " - "; echo devuelveHoraMinutos($datos_noticia['hora_fin'])?></p>
                </div>
                <div class="col-12">
                  <h3>Texto</h3>
                  <p><?php echo "$datos_noticia[texto]"?></p>
                </div>
